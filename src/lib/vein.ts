...
export const STRANDS = {
  /*
   * 上から下までの一本（2026-09-22）。区間は【名乗り】と【段から結びまで】の二つで、
   * どちらも同じ通り道（紙の左端 x=0 〜 柱の左端 x=100）の上に置いてある。
   * 継ぎ目では両側の接線を垂直にそろえてあるので、箱の縦横比が違っても折れない。
   *
   * ── なぜ二区間で、なぜ通り道に紙の縁を足したか ─────────────
   * 紙が 2,000px を切ってからは、名乗りの上の一筆と段の脈が離れていると
   * 「間」ではなく【切れている】に見える。つなぐには名乗りの帯を通すしかないが、
   * そこには大字（SUMINAWA）が紙の左端から右へ伸びている。
   * だから通り道を紙の縁（padding-inline）ぶん左へ広げ、**大字の左の余白**を
   * すり抜けさせた。名乗りの中で脈が x<12 単位（1280px の紙で 67px）に収まるのは
   * そのためで、そこから下は罫を抜いた入口の左の帯へ出て、段の横を通る。
   *
   * ── 曲率（画面の寸法へ写して測る）─────────────────────────
   *   元のハブ 1280 の最大 0.0333 /px（半径 30px）
   *   mark（箱 555×814）0.0173 /px（半径 58px）／ span（箱 555×846）0.0120 /px（83px）
   *   768px（箱 157×754）0.0076 /px ／ 390px（箱 55×839 / 55×880）0.0075・0.0047 /px
   * 直線の区間（曲率 0 の帯）は無い。
   */
  mark: [
    "M 26 0 C 21 4 11 6.5 8 11 C 5.5 15 5 23 7 29 C 9 34 14 38 19 43 C 26 49 33 55 37 62 C 40 69 41 77 40 85 C 40 90 40 95 40 100",
    "M 26.35 0 C 21.35 4 11.35 6.5 8.35 11 C 5.85 15 5.35 23 7.35 29 C 8.65 34 13.65 38 18.65 43 C 25.65 49 32.65 55 36.65 62 C 39.65 69 40.65 77 39.65 85 C 39.65 90 39.65 95 39.65 100",
    "M 25.65 0.6 C 20.65 4 10.65 6.5 7.65 11 C 5.15 15 4.65 23 6.65 29 C 9.35 34 14.35 38 19.35 43 C 26.35 49 33.35 55 37.35 62 C 40.35 69 41.35 77 40.35 85 C 40.35 90 40.35 95 40.35 99.4",
  ],
  span: [
    "M 40 0 C 40 8 48 14 58 20 C 70 27 86 34 91 45 C 94 55 90 63 82 70 C 71 80 51 88 32 94 C 25 96.5 20 99 17 100",
    "M 39.65 0 C 39.65 8 47.65 14 57.65 20 C 69.65 27 85.65 34 90.65 45 C 94.35 55 90.35 63 82.35 70 C 71.35 80 51.35 88 32.35 94 C 25.35 96.5 20.35 99 17.35 100",
    "M 40.35 0 C 40.35 8 48.35 14 58.35 20 C 70.35 27 86.35 34 91.35 45 C 93.65 55 89.65 63 81.65 70 C 70.65 80 50.65 88 31.65 94 C 24.65 96.5 19.65 99 16.65 99.4",
  ],
...
export const STRANDS_NARROW = {
  /*
   * 狭い紙の一本。通り道は【紙の縁 ＋ 段の左の余白】＝ 390px の紙で 55px しかない。
   * 振れを広い紙の 4〜5 倍に取り、x=13 単位（7px）〜 92 単位（51px）まで使う ──
   * 同じ d を流し込むと横の動きが 2px しか出ず、脈が罫にしか見えない。
   * 名乗りの帯では x<36 単位（20px ＝ 紙の縁の内側）に収め、大字に掛けない。
   */
  mark: [
    "M 55 0 C 48 3 30 5 22 9 C 14 13 12 17 16 22 C 22 28 34 33 42 40 C 52 48 58 57 58 67 C 58 78 56 90 56 100",
    "M 56.6 0 C 49.6 3 31.6 5 23.6 9 C 15.6 13 13.6 17 17.6 22 C 23.6 28 35.6 33 43.6 40 C 53.6 48 59.6 57 59.6 67 C 59.6 78 57.6 90 57.6 100",
    "M 53.4 0.5 C 46.4 3 28.4 5 20.4 9 C 12.4 13 10.4 17 14.4 22 C 20.4 28 32.4 33 40.4 40 C 50.4 48 56.4 57 56.4 67 C 56.4 78 54.4 90 54.4 99.4",
  ],
  span: [
    "M 56 0 C 56 8 66 14 76 21 C 86 29 92 37 92 47 C 92 57 86 65 76 72 C 62 81 44 89 34 95 C 31 97 30 99 30 100",
    "M 57.6 0 C 57.6 8 67.6 14 77.6 21 C 87.6 29 93.6 37 93.6 47 C 93.6 57 87.6 65 77.6 72 C 63.6 81 45.6 89 35.6 95 C 32.6 97 31.6 99 31.6 100",
    "M 54.4 0 C 54.4 8 64.4 14 74.4 21 C 84.4 29 90.4 37 90.4 47 C 90.4 57 84.4 65 74.4 72 C 60.4 81 42.4 89 32.4 95 C 29.4 97 28.4 99 28.4 99.4",
  ],
  head: [
