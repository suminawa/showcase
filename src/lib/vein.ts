...
export const STRANDS = {
  /*
   * 短い紙のための一本（2026-09-21）。
   *
   * トップは 1,900〜2,100px しか無く、段は「いま見てほしいもの」の一つだけ。
   * 段の区間（w1）と結びの区間（tail）を別の箱で継ぐと、結びの箱は罫 3 本
   * （110px）しか高さが無いのに、脈は横に 85 単位（416px）走らなければならない。
   * preserveAspectRatio="none" は箱の比でそのまま引き伸ばすので、あの区間だけ
   * 曲率が跳ね上がり、**継ぎ目に小さな半径の角ができ、その先は定規で引いた
   * 直線**になっていた（実測 最大曲率 0.0908 /px ＝ 半径 11px。元のハブの
   * 最大は 0.0333 /px ＝ 半径 30px）。
   *
   * だから区間を継がず、**段と結びのあいだを一つの箱で通す**。制御点は
   * 段の境目ではなく全長にわたってほぼ等間隔に置いてあり、右へふくらんでから
   * 左へ帰るまでが一続きの曲線になる。
   * 実測（1280px の紙・箱 489×701）最大曲率 **0.0073 /px（半径 138px）**、
   * 390px の紙（箱 52×701）で 0.0013 /px。どちらも元のハブの最大を下回る。
   * 直線の区間は無い（曲率が 0 になる点が無い）。
   */
  span: [
...
export const STRANDS_NARROW = {
  /* 狭い紙の一本。振れは広い紙の 4〜5 倍（通り道が 489px → 52px なので、
     同じ d では横の動きが 2px しか出ず、罫にしか見えない） */
  span: [
    "M 36 0 C 42 11 56 18 70 26 C 82 34 88 42 88 52 C 88 62 76 70 58 78 C 40 86 24 94 17 100",
    "M 37.6 0 C 43.6 11 57.6 18 71.6 26 C 83.6 34 89.6 42 89.6 52 C 86.4 62 74.4 70 56.4 78 C 38.4 86 22.4 94 15.4 100",
    "M 34.4 0.5 C 40.4 11 54.4 18 68.4 26 C 80.4 34 86.4 42 86.4 52 C 89.6 62 77.6 70 59.6 78 C 41.6 86 25.6 94 18.6 99.4",
  ],
  head: [
